...
void recursetry(int where) {
  int s,x1 = where % size, y1 = where / size,x2=0,y2=0;
  ...
  for(s=0; s<where; s++) {
    /* avoid division */
    if (hassquare(x1, y1, x2, y2)) goto badfail;
    if (x2 == size) { x2=0; y2++; }
    x2++;
...
int hassquare (int x1, int y1, int x2, int y2) {
  ...
  if (offset & 1) return 0;
    ...
    nx1 = x2 - offset;
    nx2 = x1 + offset;
    if (nx1 >= size || nx2 >= size || ny1 >= size || ny2 >= size) return 0;
