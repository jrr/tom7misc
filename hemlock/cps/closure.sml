...
struct
  ...
  (* number of free variables that we can safely tack onto the
     ...
     arg. we lose another because in the RTL we need one register
     free for indirect calls. see comment above *)
  val MAX_ARGS = Architecture.NREG - 3
  ...
  fun infotosl { f, fv, calls, esc, friends } =
        ...
        Fix (vael, ebody) =>
         let
           ...
           fun onefun (f, args, body) =
               ...
               in (f, subtract(free, args), direct, escapes, fns)
               end

           val fnel = map onefun vael

           val allfree = subtract(foldl (fn ((_, fr, _, _, _), s) => 
                                         union (s, fr)) nil fnel, these)

           val fns = foldr (fn ((_, _, _, _, fns), s) => fns @ s) nil fnel

           val escapes = foldl (fn ((_, _, _, esc, _), s) => 
                                union (esc, s)) nil fnel

           val directs = foldl (fn ((_, _, di, _, _), s) => 
                                union (di, s)) nil fnel
           ...
           val fs = map (fn (f, _, direct, escapes, fns) =>
                         { f=f, 
  ...
  fun determine (fns : info list) : 
      ...
      let
        ...
        (* get the representative functions for each Fix. 
           ...
           *)
        fun getreps acc mapup ((r as {f, fv, calls, esc, friends})::rest) =
            let
        ...
        fun close_c reps =
            let
              ...
              fun close_fv fvlast =
                let
                    fun close_one ({ f, fv, calls, esc, friends }, 
                                   (fvnow, changed)) =
                        let
                            val direct_calls = 
                                List.filter 
              ...
              (* now change reps: if fvnow(f) > MAX_ARGS 
                 then reps.esc = true *)
              fun spill_one (f, fv, (repnow, changed)) =
                  let 
                      val r as { fv=fvorig, calls, esc, friends, ... } = 
                                     geto "r" (V.Map.find (reps, f))
                  in
                      if length fv > MAX_ARGS 
                      then (V.Map.insert 
                            (repnow, f, { f=f, fv=fvorig, 
          ...
          fun collect e =
              case e of
                ...
                | Sumswitch (vl, va, iel, de) => 
                      ...
                      in (fns, Sumswitch(vl, va, niel, dee))
                      end
      in
          Fix(collect e)
      end
  ...
  fun convert e =
      ...
      in hoist (rewrite how NONE nil e)
      end 
