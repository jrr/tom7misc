 ...
 * representation.
 *
 * TODO: Is there any point to this? If we have order, wouldn't we always
 * want to use something smarter? If we got rid of it, we could specify
 * useful facts in the interface, like that numItems is O(1).
 *)
...
struct
  ...
  fun headi [] = NONE
    | headi ((key, value)::_) = SOME(key, value)
  ...
  fun erase (l, key) = (#1 (remove (l, key))) handle LibBase.NotFound => l
  ...
  fun foldl f init l =
    List.foldl (fn ((_, item), accum) => f(item, accum)) init l
  ...
  fun foldr f init l =
    List.foldr (fn ((_, item), accum) => f(item, accum)) init l
