...
(* This alternate imperative stream type is designed, using only types
   available at the top-level, so that imperative libraries can avoid
   a dependency on the Stream.stream source code.

   This collection of utilities is therefore intended for use by
   clients, but not by libraries. *)
