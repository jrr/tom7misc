...
struct
  ...
  fun genint uses_intinf =
    let in
      if uses_intinf
      then
        "  (* This output uses IntInf for integers internally\n" ^
      ...
      fun onemessage _ nil = "\n"
        ...
        let
          ...
          fun onefield (D.F { name, token, typ, hints }) =
            let
              val isparagraph =
                List.exists (fn D.Paragraph => true | _ => false) hints
              val isvertical = isparagraph orelse
                List.exists (fn D.Vertical => true | _ => false) hints
              ...
              fun furl i exp typ =
                ...
                | D.List t =>
                    if isvertical
                    then
                      let val nn = if isparagraph then "\\n\\n" else "\\n"
                      in
                        "($\"[" ^ nn ^ "\" ^^\n" ^
                        indent (i + 3) ^
                        "dllconcat' " ^
