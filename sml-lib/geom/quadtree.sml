...
struct
  ...
  fun map (f : 'a -> 'b) q =
    let
        fun mhoriz HEmpty = HEmpty
          | mhoriz (HPoint (l, a, x, y, r)) =
          HPoint (mvert l, f a, x, y, mvert r)
        and mvert VEmpty = VEmpty
          | mvert (VPoint (l, a, x, y, r)) =
          VPoint (mhoriz l, f a, x, y, mhoriz r)
    in
        mhoriz q
    end
  ...
  fun apppoint (f : ('a * pos) -> unit) q : unit =
    let
        fun mhoriz HEmpty = ()
          | mhoriz (HPoint (l, a, x, y, r)) = (mvert l;
                                               f (a, pos (x, y));
                                               mvert r)
        and mvert VEmpty = ()
          | mvert (VPoint (l, a, x, y, r)) = (mhoriz l;
                                              f (a, pos (x, y));
                                              mhoriz r)
    in
        mhoriz q
      ...
      fun hrec _ _ HEmpty = ()
        ...
        | hrec depth { minx, miny, maxx, maxy } (HPoint (l, a, x, y, r)) =
          ...
          in
              ...
              print "\"/>\n" (* " *);
              vrec (depth - 1) { minx = minx, maxx = x,
                                 miny = miny, maxy = maxy } l;
              vrec (depth - 1) { minx = x, maxx = maxx,
      ...
      and vrec _ _ VEmpty = ()
        ...
        | vrec depth { minx, miny, maxx, maxy } (VPoint (u, a, x, y, d)) =
          ...
          in
              ...
              print "\"/>\n" (* " *);
              hrec (depth - 1) { minx = minx, maxx = maxx,
                                 miny = miny, maxy = y } u;
              hrec (depth - 1) { minx = minx, maxx = maxx,
                                 miny = y, maxy = maxy } d
          end
