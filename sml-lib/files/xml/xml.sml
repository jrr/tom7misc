...
struct
  ...
  fun getvalue (a : Base.AttValue) =
    ...
    | Base.AV_NMTOKENS (ul : UniChar.Data list) =>
        raise XML "Unimplemented AV_NMTOKENS"
    ...
    | Base.AV_GROUP (il : int list, i : int) =>
        raise XML "Unimplemented AV_GROUP"
    | Base.AV_NOTATION (il : int list, i : int) =>
        raise XML "Unimplemented AV_NOTATION"
  ...
  fun firstleaf tag tree =
    let
      fun findy (Elem((t, attrs_ignored), [Text text])) =
        if t = tag then SOME text else NONE
        | findy (Elem((t, attrs_ignored), nil)) =
        if t = tag then SOME "" else NONE
        | findy (Text _) = NONE
        | findy (Elem(_, tl)) = findl tl
      and findl nil = NONE
        | findl (h :: t) =
        case findy h of
          NONE => findl t
