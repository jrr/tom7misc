...
% FIXME: Wait I had much better screenshots, see video/lowercase
...
on, since there was no inherent limit to the number of contours their
...
Every layer's transfer function is ``leaky relu'' ({\tt x > 0 \? x : 0.1 * x})
which is fast to compute and better than sigmoids in the output since
correct values will not just be 0 and 1. It would definitely make sense
to use convolutional layers for this problem, as features like serifs,
lines, curves, and so on could appear throughout the input and output.
I just haven't built support for that in my weird home-grown software,
yet.

I also adapted my weird home-grown software to train the
\makeuppercase\ and \makelowercase\ models simultanously. Two models
fit comfortably in GPU memory, with plenty of space for a stream of
...
from a recent round. I make sure that one model is not allowed to
get too far ahead (more than 2 rounds) of the other in training,
because I want this feedback loop to be somewhat tight.
