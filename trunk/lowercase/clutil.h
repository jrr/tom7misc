...
struct CL {
  ...
  */
  ...
  std::pair<cl_program, cl_kernel> BuildOneKernel(const std::string &kernel_src,
...
template<class T>
static std::vector<T> CopyBufferFromGPU(cl_command_queue cmd, cl_mem buf, int n) {
  std::vector<T> vec;
  vec.resize(n);
...
template<class T>
static void CopyBufferFromGPUTo(cl_command_queue cmd, cl_mem buf, std::vector<T> *vec) {
...
template<class T>
static void CopyBufferToGPU(cl_command_queue cmd, const std::vector<T> &vec, cl_mem buf) {
  CHECK_SUCCESS(clEnqueueWriteBuffer(cmd, buf, CL_TRUE, 0,
