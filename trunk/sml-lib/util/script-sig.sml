(* Collection of recipes for scripting. These are less general than
   ...
   (Requires Util, StringUtil, ListUtil, njlib, etc. XXX make CM.)
*)
signature SCRIPT =
sig
  ...
  (* Read an association list from a file. Blank lines
     are ignored. The file format, where ws means whitespace, is
     <optional ws>key-without-ws<optional ws>value<optional ws>\n
     A line with only a token on it is treated as a key with an empty
     string value. If a key is duplicated, raises Script. *)
