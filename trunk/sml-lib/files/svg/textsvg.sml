...
struct
  ...
  fun svgheaderex { x, y, width, height, generator, units } =
    let in
      ...
      " x=\"" ^ rtos x ^ units ^ "\"" ^
      " y=\"" ^ rtos y ^ units ^ "\"" ^
      " width=\"" ^ rtos width ^ units ^ "\"" ^
      " height=\"" ^ rtos height ^ units ^ "\"\n" ^
      " viewBox=\"" ^ rtos x ^ " " ^ rtos y ^ " " ^
      rtos width ^ " " ^ rtos height ^ "\"" ^
      " xml:space=\"preserve\">\n"
    end

  fun svgheader { x, y, width, height, generator } =
    svgheaderex { x = Real.fromInt x,
                  y = Real.fromInt y,
                  width = Real.fromInt width,
                  height = Real.fromInt height,
                  generator = generator,
                  units = "px" }
