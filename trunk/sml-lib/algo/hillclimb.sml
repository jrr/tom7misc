      ...
      fun shouldstop { n, a, newp, p, delta } =
        let
          fun ss nil = false
            ...
            | ss (ConvergedWithin r :: t) =
            (* why "delta" (delta <= r) orelse ss t *)
            ss t
            | ss (TimeSpent ms :: t) =
            why "time"
        ...
        in
          (* why "0 delta" (Real.== (delta, 0.0)) orelse *)
          ss stop
        end
            ...
            let
              ...
              val { newdp, new, newob, newa } =
                case (Real.compare (obplus, ob), Real.compare (obminus, ob)) of
                  (GREATER, _) => { newdp = grow dp, new = pplus,
                                    newob = obplus, newa = aplus }
                | (_, GREATER) => { newdp = grow dp, new = pminus,
                                    newob = obminus, newa = aminus }
