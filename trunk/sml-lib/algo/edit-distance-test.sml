...
struct
  ...
  fun playback s nil = s
    | playback s (Modify (i, c) :: r) =
    playback (if i >= size s then raise EditDistance "OUT"
              else ();
    ...
    | playback s (Delete i :: r) =
    playback (if i >= size s then raise EditDistance "D_OUT"
              else ();
    ...
    | playback s (Insert (i, c) :: r) =
    playback (if i > size s then raise EditDistance "D_INS"
              else ();
                CharVector.tabulate
                (size s + 1,
                 fn x =>
  ...
  fun printplan nil = print "END.\n"
    | printplan (Insert (i, c) :: r) =
    (print ("INSERT " ^ implode[c] ^ " @ " ^ Int.toString i ^ "\n");
     printplan r)
    | printplan (Modify (i, c) :: r) =
    (print ("MODIFY to " ^ implode[c] ^ " @ " ^ Int.toString i ^ "\n");
     printplan r)
    | printplan (Delete i :: r) =
    (print ("DELETE @ " ^ Int.toString i ^ "\n");
     printplan r)
  ...
  fun test (s1, s2) =
      ...
      in
        ...
        (if ss <> s2
         then raise EditDistance ("XXXXXX OH NO ! should be " ^ s2 ^ "\n")
         else ());
        (d, l)
  ...
  fun randomtest 0 = ()
    | randomtest n =
      let
        ...
        val () = if n mod 1 = 0
                 then TextIO.output (TextIO.stdErr, "[" ^ Int.toString n ^
                                     "]    Test: " ^ s1 ^ " -> " ^ s2 ^ "\n")
                 else ()
        val (d, _) = test (s1, s2)
      in
  ...
  (* XXX PERF *)
  fun vplay s nil = s
    | vplay s (Modify (i, c) :: r) =
    vplay (Vector.tabulate (Vector.length s,
                            fn x =>
                            if x = i
                            then c
                            else Vector.sub(s, x))) r
    | vplay s (Delete i :: r) =
    vplay (Vector.tabulate (Vector.length s - 1,
                            fn x =>
                            if x < i
                            then Vector.sub(s, x)
                            else Vector.sub(s, x + 1))) r
    | vplay s (Insert (i, c) :: r) =
    vplay (Vector.tabulate
           (Vector.length s + 1,
            fn x =>
            if x < i
            then Vector.sub(s, x)
  ...
  fun vtest (v1, v2) =
    let
      ...
      fun pp (Modify (i, c)) = print ("  Modify(" ^
                                      Int.toString i ^ ") -> '" ^ c ^ "'\n")
        | pp (Delete i) = print ("  Delete(" ^ Int.toString i ^ ")\n")
        | pp (Insert (i, c)) = print ("  Insert(" ^
                                      Int.toString i ^ ") @ '" ^ c ^ "'\n")
    in
      ...
      then print ("WRONG RESULT: " ^
                  String.concat (Vector.foldr op:: nil (vplay v1 plan)) ^ "\n")
      else ()
    end
