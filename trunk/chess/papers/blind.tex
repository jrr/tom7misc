...
When evaluating the first version I found that it was producing a
fairly high number of illegal positions, which was causing us to fall
back on making random moves, which is mostly boring. The second version
reduces the rate of illegal positions due to too many or too few kings.

 - we get scores with each piece
 etc.

This version, called \verb+blind_kings+, performs consistently better
than \verb+blind_yolo+ (XXX give actual record). It had an Elo World
score of XXX.
