...
struct
  ...
  fun resolvehost s =
    let
      val rh_ = _import "ml_resolvehost" private :
        string * Word32.word ref -> int ;
      val addr = ref 0w0
    in
      if rh_ (s ^ "\000", addr) = 0
      then !addr
  ...
  struct
    ...
    fun connect (a : address) (p : int) =
      let
        val to_ = _import "ml_tcp_open" private :
          Word32.word * int -> MLton.Pointer.t ;
    ...
    fun getpeeraddress r =
      let
        val ar = ref 0w0
        val pr = ref 0
        val gp_ = _import "ml_getpeeraddress" private :
          MLton.Pointer.t * Word32.word ref * int ref -> unit ;
      in
        gp_ (!!r, ar, pr);
        if !ar = 0w0
    ...
    fun send r s =
      let
        val ts_ = _import "SDLNet_TCP_Send" :
          MLton.Pointer.t * CharVector.vector * int -> int ;
      in
    ...
    fun sendarray r { array : Word8.word Array.array,
                      ...
                      num : int option } =
      let
        val ts_ = _import "ml_send_offset" private :
          MLton.Pointer.t * Word8.word Array.array * int * int -> int ;
    ...
    fun sendvec r { vec : Word8.word Vector.vector,
      ...
      let
        val ts_ = _import "ml_send_offset" private :
          MLton.Pointer.t * Word8.word Vector.vector * int * int -> int ;
    ...
    fun readbyte r =
      let
        val tr_ = _import "SDLNet_TCP_Recv" :
          MLton.Pointer.t * Word8.word ref * int -> int ;
        val buffer = ref 0w0
      in
    ...
    fun readline r =
      let
        val u =
          readuntil GC.empty readchar GC.append
