...
// PERF: Faster expressions for this?
static constexpr uint8 REV_TABLE[256] = {
...
inline uint32 Encode(uint8 byte) {
  ...
  // XXX ppuppy v2 boards are wired backwards, so we fix
  // in software here. (Make this configurable at compile time?)
  byte = REV_TABLE[byte];
