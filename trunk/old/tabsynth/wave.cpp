...
/* This is part of the DirectX SDK, and not part of the Tabsynth source
   distribution. Distribution and usage is covered by the terms of the
   DirectX SDK. */
			 ...
			 LPWAVEFILE    pWaveFile)  // Points to the struct to fill
{
  ...
  if (((hResInfo = FindResource((struct HINSTANCE__*)hModule, lpName, c_szWAV)) != NULL) &&
      ((hResData = LoadResource((struct HINSTANCE__*)hModule, hResInfo)) != NULL) &&
      ((pvRes = LockResource(hResData)) != NULL))
    {
                                // If we found it, copy the bits from the resource into
                                // our own chunk of memory
      dwSize = SizeofResource((struct HINSTANCE__*)hModule, hResInfo);
      lpMemory = malloc (dwSize); // MEMORY_New
	...
	MessageBox(0," ! waveloadresource","",MB_OK);
  return NULL;
}
 ///////////////////////////////////////////////////////////////////////////////
 //
                           ...
                           LPWAVEFILE    pWaveFile)   // Points to the struct to fill
{
  ...
  if (wave_ParseWaveMemory(lpMemory, 
    ...
    {
//	  char nuts[512];
//	sprintf(nuts,"nuts: %d",lpMemory);
//	  MessageBox(0,nuts,"Succeeding...",MB_OK);
      return lpMemory;  // OK
    }
	MessageBox(0,"Failing...","",MB_OK);
  return NULL; // Failure
}
			 ...
			 LPDWORD         lpcbWaveSize)    // Points to size
{
  ...
  if (dwRiff != mmioFOURCC('R', 'I', 'F', 'F')) {
		MessageBox(0,"Not RIFF","",MB_OK);
	  return FALSE;      // not even RIFF
  }
  if (dwType != mmioFOURCC('W', 'A', 'V', 'E')) {
		MessageBox(0,"Not WAVE","",MB_OK);
	  ...
	  if (lplpWaveHeader && !*lplpWaveHeader)
	    {
		  if (dwLength < sizeof(WAVEFORMAT)) {
			MessageBox(0,"dwlength < sizeof(waveformat)","",MB_OK);
  ...
  // of the wave
  MessageBox(0,"Finished file, not all the stuff","",MB_OK);
  return FALSE;
}


bool loadawave (char * name, int * size, WAVEFORMATEX ** wfx, BYTE ** data) {
		LPVOID wavedata;
		WAVEFILE WaveFile;
		if (!(wavedata = WAVE_LoadFile(name,&WaveFile))) return false;		
		*size = WaveFile.cbSize;
		*wfx = WaveFile.pwfxInfo;
		*data = WaveFile.pbData;
		return true;
}
