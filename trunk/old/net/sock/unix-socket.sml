...
struct
  ...
  (* Word8Array.extract for old basis library *)
  val w8a_extract = Word8ArraySlice.vector o Word8ArraySlice.slice
  ...
  fun recvVec' (SOCK s, i, {peek, oob}) =
    ...
    in
      case SockPrim.recv (s, arr, 0, i, peek, oob) of
        SockPrim.INL err => raise SockError err
      | SockPrim.INR n => w8a_extract (arr, 0, SOME n)
    end
  ...
  fun recvVecFrom' (SOCK s, i, {peek, oob}) =
    ...
    in
      case SockPrim.recvfrom (s, arr, 0, i, peek, oob) of
        SockPrim.INL err => raise SockError err
      | SockPrim.INR (n, addr, port) => (w8a_extract (arr, 0, SOME n), 
                                         INADDR (addr, port))
    end
