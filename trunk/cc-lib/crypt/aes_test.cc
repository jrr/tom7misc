...
static void test_encrypt_ecb_verbose(void) {
  ...
  for (int i = 0; i < 4; ++i) {
    AES::AES_ECB_encrypt(&ctx, plain_text + (i * 16));
...
static int test_decrypt_cbc(void) {
  ...
  AES::InitCtxIV(&ctx, key, iv);
  AES::AES_CBC_decrypt_buffer(&ctx, in, 64);
...
static int test_encrypt_cbc(void)
{
  ...
  AES::InitCtxIV(&ctx, key, iv);
  AES::AES_CBC_encrypt_buffer(&ctx, in, 64);
...
static int test_xcrypt_ctr(const char* xcrypt) {
  ...
  AES::InitCtxIV(&ctx, key, iv);
  AES::AES_CTR_xcrypt_buffer(&ctx, in, 64);
